...
# @Last Modified time: 2019-07-15 19:22:17
...
ip = input("new node's ip:")
name = input("new node's name:")
user = input("new node's user:")
# 新增节点的密码要与集群各个节点的密码一致
...
with open('/etc/hosts', mode='r') as file:
    for line in file:
        ...
        client = paramiko.SSHClient()
        client.set_missing_host_key_policy(paramiko.AutoAddPolicy())
        client.connect(hostname=_ip, username='root', password=root_password)

        if m_ip != _ip:  # 运行节点需要单独处理，以防造成无限循环
            # 为每台节点的hosts文件中添加新增节点的路由
...
# 删除本地的临时压缩包
os.system('rm -f /tmp/{hadoop_name}.tgz'.format(hadoop_name=hadoop_name))
